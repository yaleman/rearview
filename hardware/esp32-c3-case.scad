...
usb_end_wrap = 3; // Front wall overlap into the board opening on each side.
...
assert(
  (2 * usb_end_wrap) < inner_width,
  "usb_end_wrap must leave a USB opening"
);
...
union() {
  ...
  // Front corner walls wrap around the USB end by usb_end_wrap on each side.
  translate([0, 0, base_thickness])
    cube([wall_thickness + usb_end_wrap, wall_thickness, wall_height]);
  translate([
    outer_width - wall_thickness - usb_end_wrap,
    0,
    base_thickness,
  ])
    cube([wall_thickness + usb_end_wrap, wall_thickness, wall_height]);
